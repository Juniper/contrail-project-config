...
class DebRepositoryManager(RepositoryManager):
    def __init__(self, linux_release, repo_name):
        super(DebRepositoryManager, self).__init__(repo_name)
    ...
    def check_location(self):
        ...
        if not glob.glob(os.path.join(self._repo_location, '*.deb')):
            raise RuntimeError("No deb packages has been found in %s. Exiting")
    ...
    def create(self):
        ...
        Template(REPREPRO_DISTRIBUTIONS) \
            .stream(linux_release=self._linux.release) \
...
def main():
    ...
    if type_ == 'deb':
        manager = DebRepositoryManager(repository)
    elif type_ == 'rpm':
        manager = DebRepositoryManager(repository)
    else:
